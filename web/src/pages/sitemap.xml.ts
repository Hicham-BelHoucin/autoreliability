...
export const GET: APIRoute = async ({ url }) => {
  ...
  const staticRoutes = ['/', '/directory', '/rankings', '/about', '/privacy', '/terms', '/contact', '/disclaimer'];
  ...
  const entries = [
    ...staticRoutes.map((path) => `<url><loc>${xmlEscape(`${origin}${path}`)}</loc></url>`),
    ...result.rows.map((row) => {
      const path = `/reliability/${encodeURIComponent(row.make.toLowerCase())}/${encodeURIComponent(row.model.toLowerCase())}/${row.year}`;
      return `<url><loc>${xmlEscape(`${origin}${path}`)}</loc><lastmod>${new Date(row.last_synced_at).toISOString()}</lastmod></url>`;
    }),
  ];
